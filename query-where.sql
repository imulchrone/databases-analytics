...
-- 4
SELECT ACategory, TimeToFeed FROM Animal
WHERE TimeToFeed >= 1.5 AND TimeToFeed <= 2.75;

-- 5
SELECT AName, TimeToFeed FROM Animal
WHERE TimeToFeed IN ((SELECT MIN(TimeToFeed) FROM Animal), (SELECT MAX(TimeToFeed) FROM Animal));
